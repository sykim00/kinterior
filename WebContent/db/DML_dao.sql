...
-- 4. 글번호(cid)로 전체내용(consultantDto) 가져오기
...
-- 6. 글 삭제하기
    ...
    -- 6번글 (6-1 지우기)
    DELETE FROM CONSULTANT WHERE cGROUP = 6 AND (cSTEP >=1 AND cSTEP < (SELECT NVL(MIN(cSTEP),99) FROM CONSULTANT WHERE cGROUP = 6 AND cSTEP > 1 AND cINDENT <=1));
-- 7. 답변글 쓰기 전단계
UPDATE CONSULTANT SET cSTEP = cSTEP + 1 WHERE cGROUP = 3 AND cSTEP > 0;
-- 8. 답변글 쓰기
INSERT INTO CONSULTANT (cID, mID, aID, cTITLE, cCONTENT, cFILENAME, cGROUP, cSTEP, cINDENT, cIP)
    VALUES (CONSULTANT_SEQ.NEXTVAL, null, 'admin', '애견카페 견적문의 드립니다. 답변글입니다.', '글 본문입니다.', null, 6, 1, 1, '185.854.3.13');
INSERT INTO CONSULTANT (cID, mID, aID, cTITLE, cCONTENT, cFILENAME, cGROUP, cSTEP, cINDENT, cIP)
    VALUES (CONSULTANT_SEQ.NEXTVAL, 'fff', null, '애견카페 견적문의 드립니다. 답의 답변글입니다.', '글 본문입니다.', '3.xlsx', 6, 2, 2, '185.854.3.13');
...
INSERT INTO REVIEW_BOARD (rID, mID, rTITLE, rCONTENT, rPHOTO, rIP)
    VALUES (REVIEWBOARD_SEQ.NEXTVAL,'ddd', '헌집이 새집 되었어요', 
            ...
            '4.jpg' , '142.512.21.0');
-- 4. 글번호(rid)로 전체내용(consultantDto) 가져오기
...
-- 7. 회원탈퇴시 탈퇴하는 회원(mid)이 쓴 글 모두 삭제하기
...
-- 4. 글번호(pid)로 전체내용(consultantDto) 가져오기
...
-- 3. 글번호(nID)로 전체내용(consultantDto) 가져오기
